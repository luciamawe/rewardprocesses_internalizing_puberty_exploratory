...
confirmatory2_gender_CBCLrawscore_excludingoutliers  <- lmer(cbcl_scr_syn_internal_r ~ PDS_score*sex +
                      race.eth.7level +
                      high.educ +
                      household.income +
                      married.or.livingtogether +
                      ...
                      data =  subset(PDS_correct, 
                                          cbcl_scr_syn_internal_r_z > -3 & cbcl_scr_syn_internal_r_z < 3))
summary(confirmatory2_gender_CBCLrawscore_excludingoutliers)

plot_model(confirmatory2_gender_CBCLrawscore_excludingoutliers,type = "est", show.data = FALSE) #Forest plot

plot_model(confirmatory2_gender_CBCLrawscore_excludingoutliers,type = "int", terms = "PDS_score", 
           show.data = TRUE, jitter = 0.1, line.size = 2) +
   ylab("Internalizing Score") +
   xlab("PDS Stage") +
...
file.name = (here("output","question2_exploratorysample_excludingCBLCoutliers.tiff"))
ggsave(file.name,plot = last_plot(), dpi = 150, device = "tiff")
```
