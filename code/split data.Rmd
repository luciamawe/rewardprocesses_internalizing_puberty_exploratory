...
ABCD_Data_baseline <- ABCD_Data_baseline %>% filter(sex.x!="") #remove 6 participants with no gender
nrow(ABCD_Data_baseline)
```
...
reduced.ABCD.table<- ABCD_Data_baseline %>% 
  select(interview_age.x,sex.x,cbcl_scr_syn_internal_t,hormone_scr_ert_mean)

table <- compareGroups(sex.x ~ ., data = reduced.ABCD.table,na.omit(TRUE))
...
ABCD_Data_baseline$Z <- block_ra(blocks = c(ABCD_Data_baseline$sex.x))
table(ABCD_Data_baseline$Z, ABCD_Data_baseline$sex.x)

reduced.ABCD.table<- ABCD_Data_baseline %>% 
  select(interview_age.x,sex.x,cbcl_scr_syn_internal_t,hormone_scr_ert_mean,Z)
...
table <- compareGroups(sex.x ~ ., data =reduced.ABCDexp.table,na.omit(TRUE))
pvals <- getResults(table, "p.overall")
p.adjust(pvals, method = "BH")
export_table <- createTable(table)
export_table
