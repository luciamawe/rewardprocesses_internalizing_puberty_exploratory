...
script_path <- file.path(data_dir,"rewardprocesses_internalizing_puberty_exploratory","code","define_dataframes.R") #exploratory works for both versions
source(script_path)
```
...
cor.test(data$accumbens_rvsn_ant_z,data$caudate_rvsn_ant_z) # Conf 0.59.
cor.test(data$accumbens_rvsn_ant_z,data$putamen_rvsn_ant_z) # Conf 0.51.
cor.test(data$caudate_rvsn_ant_z,data$putamen_rvsn_ant_z) # Exp 0.851. Conf 0.79
...
sum(is.na(dataformodel$race.ethnicity.5level)) #Exp 41 missing, Conf 49 missing
sum(is.na(dataformodel$demo_race_hispanic))  #Exp 50 missing, Conf 43 missing
sum(is.na(dataformodel$high.educ))  #Exp 3, Conf 7
sum(is.na(dataformodel$household.income)) #Exp 293, Conf 298
sum(is.na(dataformodel$married.or.livingtogether))  #Exp 22, Conf 24 
sum(is.na(dataformodel$fam_history_q6d_depression)) #Exp 1937 missing!, Conf 2002
...
exploratory4d_lg_neutral_RT_females <- gamm4(cbcl_scr_syn_internal_r ~ 
                            PDS_score*
...
summary(exploratory4d_lg_neutral_RT_females$gam)
```
```{r Exploratory4D, echo = FALSE}
#FEMALES
dataformodel <- data_no_RT_MID_outliers_females
...
#LARGE REWARD VS. NEUTRAL POSITIVE
exploratory4d_lg_neutral_RT_males <- gamm4(cbcl_scr_syn_internal_r ~ 
                            PDS_score*
                            rt_diff_large_neutral_z+
                            #sex+
                            race.ethnicity.5level +
...
summary(exploratory4d_lg_neutral_RT_males$gam)
```
```{r}
#LARGE REWARD VS. SMALL REWARD POSITIVE
exploratory4d_lg_small_RT_males <- gamm4(cbcl_scr_syn_internal_r ~ 
                            PDS_score*
                            rt_diff_large_small_z+
                            #sex+
                            race.ethnicity.5level +
...
summary(exploratory4d_lg_small_RT_males$gam)
...
coef_test(confirmatory4_accumbens_ant_females$mer, vcov = "CR2", coeffs = "all")
# beta = 0.004; t= 0.01; p = 0.99.
```
...
coef_test(confirmatory4_accumbens_ant_males$mer, vcov = "CR2", coeffs = "all")
# beta = 0.37; t= 0.90; p = 0.38.
```
...
coef_test(exploratory4a_accumbens_feedback_females$mer, vcov = "CR2", coeffs = "all")
# beta = 0.004; t= 0.01; p = 0.99.
```
```
...
```
###Males
```{r Exploratory4Caudate_Males, echo = FALSE}
...
```
###Males
```{r Exploratory4Putamen_Males, echo = FALSE}
