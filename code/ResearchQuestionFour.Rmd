...
PDS_correct <- subset(data, PDS_score < 5) #Be mindful that PDS category goes from 1 to 5, while PDS_average goes from 1 to 4 (continuous)  #4224

#Separate by sex
PDS_correct_females <- subset(PDS_correct, sex == "F")  #2059
PDS_correct_males <- subset(PDS_correct, sex == "M")    #2165

#Data excluding outliers in striatal activity and cbcl 
data = subset(PDS_correct, striatum_rvsn_ant_z > -3 & striatum_rvsn_ant_z < 3 & cbcl_scr_syn_internal_r_z > -3 & cbcl_scr_syn_internal_r_z < 3)  #4097
```
...
sum(is.na(data$src_subject_id)) #0 missing  
sum(is.na(data$sex))  #0 missing   
sum(is.na(data$mri_info_deviceserialnumber)) #0 missing 
sum(is.na(data$rel_family_id)) #0 missing 
sum(is.na(data$race.ethnicity.5level)) #41
sum(is.na(data$demo_race_hispanic))  #50
sum(is.na(data$high.educ))  #3
sum(is.na(data$household.income)) #293
sum(is.na(data$married.or.livingtogether))  #22
sum(is.na(data$fam_history_q6d_depression)) #1937 missing!
...
#Explore ethnicity variables
```{r}
fulldata$race.eth.7level
table(fulldata$race.eth.7level)
#Low prevalence of NHPI n=6, AIAN=22
```
...
tempdata <- data[which(complete.cases(data[,c("race.eth.7level",
                            "demo_race_hispanic",
                            "high.educ",
                            "household.income",
                            "married.or.livingtogether",
