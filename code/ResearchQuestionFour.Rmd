...
# Covariates.
#      A. Random effects.
#           1. Family. # Need to figure out what variable this is. rel_family_id?
#           2. Site. (site_id_l)
#           3. Subject. (src_subject_id) # There is only one timepoint so we cannot do a random effect for each participant.
#           4. Device. # Need to figure out what variable this is. On DEAP: mri_info_device.serial.number; not sure if we have this in the data or if this                is important.
# 

          #  Random effects are nested: (1|abcd_site/device/rel_family_id) [unless kids went in different scanners?]

#      B. Fixed effects.
#           1. Race parent. (race.eth.8level); check that this is the one we want to use.
...
data$demo_ethn_v2 <- as.factor(data$demo_ethn_v2)
...
# PDS_correct <- subset(PDS, PDS_sum_both < 20)


# Check the PDS scores.
...
PDS_correct <- subset(data, PDS_sum_both < 20)

# Separate analyses first.
confirmatory4_accumbens <- lm(cbcl_scr_syn_internal_t ~ PDS_score_both*accumbens_rvsn_ant + 
                            race.eth.8level +
                            demo_ethn_v2 +
                            high.educ +
                            household.income +
...
confirmatory4_caudate <- lm(cbcl_scr_syn_internal_t ~ PDS_score_both*caudate_rvsn_ant + 
                            race.eth.8level +
                            demo_ethn_v2 +
                            high.educ +
                            household.income +
...
confirmatory4_putamen <- lm(cbcl_scr_syn_internal_t ~ PDS_score_both*putamen_rvsn_ant + 
                            race.eth.8level +
                            demo_ethn_v2 +
                            high.educ +
                            household.income +
...
confirmatory4_compositestriatum <- lm(cbcl_scr_syn_internal_t ~ PDS_score_both*striatum_rvsn_ant + 
                            race.eth.8level +
                            demo_ethn_v2 +
                            high.educ +
                            household.income +
...
# Separate analyses first.
exploratory4A_accumbens<- lm(cbcl_scr_syn_internal_t ~ PDS_score_both*accumbens_posvsneg_feedback + 
                            race.eth.8level +
                            demo_ethn_v2 +
                            high.educ +
                            household.income +
...
exploratory4A_caudate<- lm(cbcl_scr_syn_internal_t ~ PDS_score_both*caudate_posvsneg_feedback + 
                            race.eth.8level +
                            demo_ethn_v2 +
                            high.educ +
                            household.income +
...
exploratory4A_putamen<- lm(cbcl_scr_syn_internal_t ~ PDS_score_both*putamen_posvsneg_feedback + 
                            race.eth.8level +
                            demo_ethn_v2 +
                            high.educ +
                            household.income +
...
# Composite striatum score.
exploratory4A_compositestriatum <- lm(cbcl_scr_syn_internal_t ~ PDS_score_both*striatum_posvsneg_feedback + 
                            race.eth.8level +
                            demo_ethn_v2 +
                            high.educ +
                            household.income +
