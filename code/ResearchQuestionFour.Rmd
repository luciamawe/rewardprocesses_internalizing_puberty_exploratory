...
#Remove outliers (you need to type "YES" into the console)
source("https://goo.gl/4mthoF")
outlierKD(data, accumbens_rvsn_ant)
outlierKD(data, caudate_rvsn_ant)
outlierKD(data, putamen_rvsn_ant)
outlierKD(data, striatum_rvsn_ant)
boxplot(data$striatum_rvsn_ant)

boxplot(data$cbcl_scr_syn_internal_r)

data$cbcl_scr_syn_internal_r_z <- scale(data$cbcl_scr_syn_internal_r)

#outlierKD(data, cbcl_scr_syn_internal_r) #should we remove CBCL outliers?
...
interact_plot(confirmatory4_compositestriatum,
              pred = PDS_score, 
              modx=striatum_rvsn_ant_z, 
              plot.points = TRUE,
              x.label = "PDS score",
              y.label = "Internalizing symptoms",
              main.title = "",
...
p <- plot_model(confirmatory4_compositestriatum_excludingoutliers,type = "int", terms = "PDS_score",            show.data = TRUE, jitter = 0.1, line.size = 2) +
   ylab("Internalizing Score") +
   xlab("Average PDS score") +
   labs(colour = "MID Striatal activity z-score \n (reward anticipation vs neutral)")+
   #scale_y_continuous(breaks = seq(0, 100, by = 10), limits = c(30,100)) +
...
interact_plot(confirmatory4_compositestriatum_excludingoutliers,
              pred = PDS_score, 
              modx=striatum_rvsn_ant_z, 
              plot.points = TRUE,
              x.label = "PDS score",
