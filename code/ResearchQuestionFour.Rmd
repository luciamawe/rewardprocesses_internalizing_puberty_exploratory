...
sum(is.na(data$src_subject_id))
sum(is.na(data$sex))
sum(is.na(data$mri_info_deviceserialnumber))
sum(is.na(data$rel_family_id))
...
```{r}
#model comparison, core model has n = 4097, and model with covariates has n = 2003. best way to extrac the same dataset to allow for comparisons?

data_for_models<-subset(PDS_correct, striatum_rvsn_ant_z > -3 & striatum_rvsn_ant_z < 3 & cbcl_scr_syn_internal_r_z > -3 & cbcl_scr_syn_internal_r_z < 3)

data_complete <- data_for_models[complete.cases(project_specific_s1s6$osem_negative_score), ] ## I've uses this in the past, not sure here_
