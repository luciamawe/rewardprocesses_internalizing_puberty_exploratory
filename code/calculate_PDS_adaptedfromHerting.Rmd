...
## Megan has used DEAP names, we are using NDA names. 
...
##Reduce to baseline data only N=11,883
#df<- subset(df,eventname=="baseline_year_1_arm_1")   #We are scoring all waves

##Look at data formats
str(df)
describeBy(df, group=df$sex) #sex is sex for DEAP
...
df_caregiver <- df[caregivervars] #11883
head(df_caregiver)
```
...
##Remove 0 people with blank sex  #no NAs for 3.0, there were 6 NAs in 2.0

df_caregiver<-df_caregiver %>% drop_na(sex)
df_caregiver %>% dplyr::count(sex)
```

### Check some more sex/sex matching
```{r}
#LMW commented this whole section out since we do not have pubertal_sex_p in nda30
# #PDS vs. sex check
# ##Who has a PDS sex variable of "NA" and remove
# 
# df_caregiver %>% dplyr::count(pubertal_sex_p) ##pubertal_sex_p is missing in 3.0 release, 2.1 had 5 NAs
# 
# df_caregiver=df_caregiver[-c(which(is.na(df_caregiver$pubertal_sex_p))),]
# 
# table(df_caregiver$sex,df_caregiver$pubertal_sex_p) ##Mismatch =63 
# 
# 
# ##Remove mismatches
# df_caregiver <- df_caregiver[-c(which(df_caregiver$sex == "M" & df_caregiver$pubertal_sex_p == "Female"), which(df_caregiver$sex == "F" & df_caregiver$pubertal_sex_p == "Male")),]
# 
# ##Check this worked and rerun descriptives
# table(df_caregiver$sex,df_caregiver$pubertal_sex_p) #N=5659 F and N=6142 M
# 
# describeBy(df_caregiver, group=df_caregiver$sex)
# 
# ##Note NDAR_INV5D3CU8ML has Male sex but female answers -- we will remove from dataset
# c(which(df_caregiver$sex == "M" & !is.na(df_caregiver$pds_f4_p)))
# df_caregiver=df_caregiver[-c(which(df_caregiver$sex == "M" & !is.na(df_caregiver$pds_f4_p))),]
# 
# ##Final sample for PDS caregiver reporting = N=6141 MALES AND N=5659 FEMALES
# xtabs(~df_caregiver$sex+df_caregiver$pubertal_sex_p)
...
# df_caregiver$pds_1_p_num[df_caregiver$pds_1_p_num == 5] <- NA
# df_caregiver$pds_2_p_num=as.numeric(df_caregiver$pds_2_p)
# df_caregiver$pds_2_p_num[df_caregiver$pds_2_p_num == 5] <- NA
# df_caregiver$pds_3_p_num=as.numeric(df_caregiver$pds_3_p)
# df_caregiver$pds_3_p_num[df_caregiver$pds_3_p_num == 5] <- NA
# df_caregiver$pds_m4_p_num=as.numeric(df_caregiver$pds_m4_p)
# df_caregiver$pds_m4_p_num[df_caregiver$pds_m4_p_num == 5] <- NA
# df_caregiver$pds_m5_p_num=as.numeric(df_caregiver$pds_m5_p)
# df_caregiver$pds_m5_p_num[df_caregiver$pds_m5_p_num == 5] <- NA
# df_caregiver$pds_f4_p_num=as.numeric(df_caregiver$pds_f4_p)
# df_caregiver$pds_f4_p_num[df_caregiver$pds_f4_p_num == 5] <- NA

##PDS Recode categorical into variables with labels so we can sum and average; Note "I don't know 999 is not numerically meaningful" so turned into NA for just these variables
df_caregiver$pds_1_p_num=as.numeric(df_caregiver$pds_1_p)
...
#Megan has the following lines which I do not think we need, as we already have the numbers

# df_caregiver$pds_f5b_p_num[df_caregiver$pds_f5b_p == "Yes"] <- 4
# df_caregiver$pds_f5b_p_num[df_caregiver$pds_f5b_p == "No"] <- 1
...
# df_caregiverm=subset(df_caregiver, pubertal_sex_p=="Male")
# df_caregiverf=subset(df_caregiver, pubertal_sex_p=="Female")
...
# xtabs(~df_caregiver$pubertdev_males_valid_for_avg+df_caregiver$pubertal_sex_p)
# xtabs(~df_caregiver$pubertdev_females_valid_for_avg+df_caregiver$pubertal_sex_p)
...
##How many  missing
# xtabs(~df_caregiver$pds_y_ss_female_cat_nm_p+df_caregiver$pubertal_sex_p)
# xtabs(~df_caregiver$pds_p_ss_male_cat_nm+df_caregiver$pubertal_sex_p)
...
# PDSCaregivervars <- c("src_subject_id", "sex", "eventname", "interview_age", "sex", "pubertdev_Avg_p_all","pds_p_ss_cat_all", "pubertdev_gonadAvg_p_all", "pubertdev_adrenalAvg_p","pds_1_p","pds_2_p", "pds_3_p", "pds_m4_p", "pds_m5_p", "pds_f4_p", "pds_f5b_p", "pds_1_p_num","pds_2_p_num", "pds_3_p_num", "pds_m4_p_num", "pds_m5_p_num", "pds_f4_p_num", "pds_f5b_p_num")
...
##STEP 1: Clean mis-match of data -- sex = sex at birth/sex matching
```{r}
# ##Remove 6 people with blank sex
# # df_youth<- df_youth[-which(is.na(df_youth$sex)), ]
# # df_youth %>% dplyr::count(sex)
# 
# ### Check some more sex/sex matching
# 
# #PDS vs. sex check
...
# 
# df_youth %>%
# dplyr::count(pds_sex_y) ##20
# df_youth=df_youth[-c(which(is.na(df_youth$pds_sex_y))),]
# 
# table(df_youth$sex,df_youth$pds_sex_y) ##Mismatch 63 
# 
# ##Remove mismatches
# df_youth <- df_youth[-c(which(df_youth$sex == "M" & df_youth$pds_sex_y == "Female"), which(df_youth$sex == "F" & df_youth$pds_sex_y == "Male")),]
# 
# ##Check this worked and rerun descriptives
# table(df_youth$sex,df_youth$pds_sex_y) #N=5628 F and N=6158 M
# 
# describeBy(df_youth, group=df_youth$sex_y)
# 
# ##Final sample for PDS youth reporting #N=5628 F and N=6158 M
# table(df_youth$sex,df_youth$pds_sex_y) 
...
#Megan has the following lines which I do not think we need, as we already have the numbers
#df_youth$pds_f5_y_num[df_youth$pds_f5_y == "Yes"] <- 4
#df_youth$pds_f5_y_num[df_youth$pds_f5_y == "No"] <- 1
...
##Count who is valid based on sex 
# xtabs(~df_youth$pubertdev_males_valid_for_avg+df_youth$pds_sex_y)
# xtabs(~df_youth$pubertdev_females_valid_for_avg+df_youth$pds_sex_y)
...
##Check category score already in NDAR dataset
##How many  missing
# xtabs(~df_youth$pds_y_ss_female_cat_nm+df_youth$pds_sex_y)
# xtabs(~df_youth$pds_y_ss_male_cat_nm+df_youth$pds_sex_y)
...
####RETAKE HERE. MALE YOUTH GONADAL NOT WORKING
#Compute male gonad avg
...
# PDSYouthvars <- c("src_subject_id", "sex", "eventname", "demo_prim_p", "interview_age", "pds_sex_y", "pubertdev_Avg_all","pds_y_ss_cat_all", "pubertdev_gonadAvg_all", "pubertdev_adrenalAvg","pds_ht2_y","pds_bdyhair_y", "pds_skin2_y", "pds_m4_y", "pds_m5_y", "pds_f4_2_y", "pds_f5_y", "pds_ht2_y_num","pds_bdyhair_y_num", "pds_skin2_y_num", "pds_m4_y_num", "pds_m5_y_num", "pds_f4_2_y_num", "pds_f5_y_num")
...
#Caregiver baseline plots
```{r}
##Reduce to baseline data only N=11,883
df_caregiver <- subset(df_caregiver,eventname=="baseline_year_1_arm_1")   #We are scoring all waves

##Plot the data
M_Avg=ggplot(subset(df_caregiver, sex=="Male"),aes(x=pubertdev_maleAvg_p))+geom_histogram(position="dodge",na.rm = TRUE,alpha=0.5,binwidth=0.25)+theme_bw()
F_Avg=ggplot(subset(df_caregiver, sex=="Female"),aes(x=pubertdev_femaleAvg_p))+geom_histogram(position="dodge",na.rm = TRUE,alpha=0.5,binwidth=0.25)+theme_bw()

Both_Avg=ggplot(df_caregiver,aes(x=pubertdev_Avg_p_all,group=sex,fill=sex))+geom_histogram(na.rm = TRUE,alpha=0.5,binwidth=0.25)+theme_bw()
...
AvgPDS=ggplot2.histogram(data=df_caregiver, xName='pubertdev_Avg_p_all',binwidth=0.2,
    groupName='sex', legendPosition="top",
        alpha=0.3, position="identity")+theme_bw()+theme(axis.text.x  = element_text(size=12), axis.text.y  = element_text(size=12))+theme( axis.title.x=element_blank(), axis.title.y=element_blank())+facet_grid(~sex)
ggsave(AvgPDS,  file=here("output","AvgPDS.tiff"), width = 12, height = 8, units = "in",
  dpi = 300)

ggplot2.histogram(data=df_caregiver, xName='pubertdev_Avg_p_all',addDensity=TRUE,groupName='sex', legendPosition="top",alpha=0.5,addMeanLine=TRUE, meanLineColor="white", meanLineSize=1.5, binwidth=.2)+theme_bw()


PubertyCategory=ggplot(subset(df_caregiver, !is.na(pds_p_ss_cat_all)),aes(x=factor(pds_p_ss_cat_all,levels=c("Pre", "Early", "Mid", "Late","Post")),group=sex,fill=sex))+geom_histogram(stat="count",position="dodge",na.rm = TRUE,alpha=0.5,binwidth=0.25)+theme_bw()+theme(axis.text.x  = element_text(size=12), axis.text.y  = element_text(size=12))+theme( axis.title.x=element_blank(), axis.title.y=element_blank())+facet_grid(~sex)
PubertyCategory

ggsave(PubertyCategory,  file=here("output","PubertyCategory.tiff"), width = 12, height = 8, units = "in",
  dpi = 300)

##Plot the data
Avg_adrenal=ggplot(df_caregiver,aes(x=pubertdev_adrenalAvg_p,group=sex,fill=sex))+geom_histogram(position="dodge",na.rm = TRUE,alpha=0.5,binwidth=0.25)+theme_bw()
...
ggsave(AdrenalAvg,  file=here("output","AdrenalAvg.tiff"), width = 12, height = 8, units = "in",
  dpi = 300)
...
ggsave(GonadalAvg,  file=here("output","GonadalAvg.tiff"), width = 12, height = 8, units = "in",
  dpi = 300)

ggplot2.histogram(data=df_caregiver, xName='pubertdev_adrenalAvg_p',addDensity=TRUE,groupName='sex', legendPosition="top",alpha=0.5,addMeanLine=TRUE, meanLineColor="white", meanLineSize=1.5, binwidth=.5)+theme_bw()

ggplot2.histogram(data=df_caregiver, xName='pubertdev_gonadAvg_p_all',addDensity=TRUE,groupName='sex', legendPosition="top",alpha=0.5,addMeanLine=TRUE, meanLineColor="white", meanLineSize=1.5, binwidth=.5)+theme_bw()

```

#Youth baseline plots
```{r}
df_youth <- subset(df_youth,eventname=="baseline_year_1_arm_1")   #We are scoring all waves

##Plot the data
M_Avg_y=ggplot(subset(df_youth, sex=="Male"),aes(x=pubertdev_maleAvg))+geom_histogram(position="dodge",na.rm = TRUE,alpha=0.5,binwidth=0.25)+theme_bw()
F_Avg_y=ggplot(subset(df_youth, sex=="Female"),aes(x=pubertdev_femaleAvg))+geom_histogram(position="dodge",na.rm = TRUE,alpha=0.5,binwidth=0.25)+theme_bw()

Both_Avg_y=ggplot(df_youth,aes(x=pubertdev_Avg_all,group=sex,fill=sex))+geom_histogram(na.rm = TRUE,alpha=0.5,binwidth=0.25)+theme_bw()
...
install_github("kassambara/easyGgplot2")
library(easyGgplot2)
AvgPDS_y=ggplot2.histogram(data=df_youth, xName='pubertdev_Avg_all',binwidth=0.2,
    groupName='sex', legendPosition="top",
        alpha=0.3, position="identity")+theme_bw()+theme(axis.text.x  = element_text(size=12), axis.text.y  = element_text(size=12))+theme( axis.title.x=element_blank(), axis.title.y=element_blank())+facet_grid(~sex)
ggsave(AvgPDS,  file=here("output","AvgPDS_youth.tiff"), width = 12, height = 8, units = "in",
  dpi = 300)

ggplot2.histogram(data=df_youth, xName='pubertdev_Avg_all',addDensity=TRUE,groupName='sex', legendPosition="top",alpha=0.5,addMeanLine=TRUE, meanLineColor="white", meanLineSize=1.5, binwidth=.2)+theme_bw()

PubertyCategory_y=ggplot(subset(df_youth, !is.na(pds_y_ss_cat_all)),aes(x=factor(pds_y_ss_cat_all,levels=c("Pre", "Early", "Mid", "Late","Post")),group=sex,fill=sex))+geom_histogram(stat="count",position="dodge",na.rm = TRUE,alpha=0.5,binwidth=0.25)+theme_bw()+theme(axis.text.x  = element_text(size=12), axis.text.y  = element_text(size=12))+theme( axis.title.x=element_blank(), axis.title.y=element_blank())+facet_grid(~sex)


ggsave(PubertyCategory_y,  file=here("output","PubertyCategory_Youth.tiff"), width = 12, height = 8, units = "in",
  dpi = 300)

##Plot the data
Avg_adrenal=ggplot(df_youth,aes(x=pubertdev_adrenalAvg,group=sex,fill=sex))+geom_histogram(position="dodge",na.rm = TRUE,alpha=0.5,binwidth=0.25)+theme_bw()

AdrenalAvg=ggplot2.histogram(data=df_youth, xName='pubertdev_adrenalAvg' ,binwidth=0.5,
    groupName='sex', legendPosition="top",
        alpha=0.3, position="identity")+theme_bw()+theme(axis.text.x  = element_text(size=12), axis.text.y  = element_text(size=12))+theme( axis.title.x=element_blank(), axis.title.y=element_blank())+facet_grid(~sex)

ggsave(AdrenalAvg,  file=here("output","AdrenalAvg_Youth.tiff"), width = 12, height = 8, units = "in",
  dpi = 300)
...
ggsave(GonadalAvg,file=here("output","GonadalAvg_Youth.tiff"), width = 12, height = 8, units = "in",
  dpi = 300)

ggplot2.histogram(data=df_youth, xName='pubertdev_adrenalAvg',addDensity=TRUE,groupName='sex', legendPosition="top",alpha=0.5,addMeanLine=TRUE, meanLineColor="white", meanLineSize=1.5, binwidth=.5)+theme_bw()

ggplot2.histogram(data=df_youth, xName='pubertdev_gonadAvg_all',addDensity=TRUE,groupName='sex', legendPosition="top",alpha=0.5,addMeanLine=TRUE, meanLineColor="white", meanLineSize=1.5, binwidth=.5)+theme_bw()
```