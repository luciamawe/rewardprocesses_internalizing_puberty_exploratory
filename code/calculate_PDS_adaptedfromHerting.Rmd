...
##Plot the data
M_Avg=ggplot(subset(df_caregiver, sex=="M"),aes(x=pubertdev_maleAvg_p))+geom_histogram(position="dodge",na.rm = TRUE,alpha=0.5,binwidth=0.25)+theme_bw()
F_Avg=ggplot(subset(df_caregiver, sex=="F"),aes(x=pubertdev_femaleAvg_p))+geom_histogram(position="dodge",na.rm = TRUE,alpha=0.5,binwidth=0.25)+theme_bw()
...
PDSCaregivervars <- c("src_subject_id", "sex", "eventname", "demo_prim_p", "interview_age", "pubertal_sex_p", "pubertdev_Avg_p_all","pds_p_ss_cat_all", "pubertdev_gonadAvg_p_all", "pubertdev_adrenalAvg_p","pds_1_p","pds_2_p", "pds_3_p", "pds_m4_p", "pds_m5_p", "pds_f4_p", "pds_f5b_p", "pds_1_p_num","pds_2_p_num", "pds_3_p_num", "pds_m4_p_num", "pds_m5_p_num", "pds_f4_p_num", "pds_f5b_p_num")
...
describeBy(df_youth, group=df_youth$sex)
...
#This will give total number of NAs in males per pubertdev column
apply(subset(df_youth, sex=="M")[,c("pds_ht2_y_num","pds_bdyhair_y_num", "pds_skin2_y_num", "pds_m4_y_num", "pds_m5_y_num")], 2, function(x) sum(is.na(x)))
#And total number of NAs in females per pubertdev column
apply(subset(df_youth, sex=="F")[,c("pds_ht2_y_num","pds_bdyhair_y_num", "pds_skin2_y_num", "pds_f4_2_y_num", "pds_f5_y_num")], 2, function(x) sum(is.na(x)))
...
##Plot the data
M_Avg=ggplot(subset(df_youth, sex=="Male"),aes(x=pubertdev_maleAvg))+geom_histogram(position="dodge",na.rm = TRUE,alpha=0.5,binwidth=0.25)+theme_bw()
F_Avg=ggplot(subset(df_youth, sex=="Female"),aes(x=pubertdev_femaleAvg))+geom_histogram(position="dodge",na.rm = TRUE,alpha=0.5,binwidth=0.25)+theme_bw()

Both_Avg=ggplot(df_youth,aes(x=pubertdev_Avg_all,group=sex,fill=sex))+geom_histogram(na.rm = TRUE,alpha=0.5,binwidth=0.25)+theme_bw()
...
PubertyCategory=ggplot(subset(df_youth, !is.na(pds_y_ss_cat_all)),aes(x=factor(pds_y_ss_cat_all,levels=c("Pre", "Early", "Mid", "Late","Post")),group=sex,fill=sex))+geom_histogram(stat="count",position="dodge",na.rm = TRUE,alpha=0.5,binwidth=0.25)+theme_bw()+theme(axis.text.x  = element_text(size=12), axis.text.y  = element_text(size=12))+theme( axis.title.x=element_blank(), axis.title.y=element_blank())+facet_grid(~sex)
...
df_youth$pubertdev_males_valid_for_gonad<-ifelse(df_youth$sex=="M" & df_youth$pubertdev_malegonad_Qcount==3, 1, 0)
